...
export function WaterMapCanvas({
    ...
    useEffect(() => {
        ...
        // Recreate marker на каждое pin change — pin-drop + ripple animation
        // replays при каждом setPin (production-feature по claude design p236).
        // Альтернатива setLngLat + restart-animation hack оставляет moment
        // когда marker visually «подъезжает» из (0,0) к новым координатам.
        // Recreate чище — old marker remove, new создаётся уже на правильной
        // позиции, animation сразу с правильного visual origin.
        //
        // Maplibre.Marker применяет inline `transform: translate(Xpx, Ypx)`
        // на ПЕРЕДАННЫЙ element для позиционирования. Поэтому animation
        // НЕ может trogать transform на outer `.wm-pin` — перезатрёт
        // positioning. SVG обёрнут в `.wm-pin-inner` div, animation на нём.
        pinMarkerRef.current?.remove();
