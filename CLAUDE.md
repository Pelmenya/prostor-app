...
**Паттерн Stale-While-Revalidate (SWR):**

- Показываем локальные данные мгновенно (stale)
- В фоне подтягиваем свежие с сервера (revalidate)
- Если данные изменились — обновляем UI
- TanStack Query делает `refetchOnWindowFocus` по умолчанию — используем это
...
**Тест-кейсы для мультиклиентных фич:**

- Изменить на устройстве A → переключиться на B → данные обновились
- Быстро изменить + переключиться (пока debounce pending) → локальные данные не потерялись
- Оффлайн → изменить → онлайн → синхронизировалось
...
**При изменении NEXT*PUBLIC*\* нужен пересбор образа** (`docker compose build`).
...
- Автодеплой main на прод (docker compose up -d --build)
...
- Переменные с префиксом `NEXT_PUBLIC_` — доступны на клиенте, инлайнятся при билде
...
Агенты из `.claude/agents/` совпадают по имени со встроенными типами субагентов. При запуске указывай `subagent_type` по имени файла:
...
**Важно:** при вызове Agent tool использовать `subagent_type` из таблицы выше, а не `general-purpose`. Все агенты используют модель `opus`.

Рекомендуется запускать code review и архитектурное ревью **перед каждым PR**.
